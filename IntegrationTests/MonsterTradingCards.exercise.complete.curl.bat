...
REM --------------------------------------------------
echo 22) battle with different cards
echo Assign new deck to kienboec (all dragons ... guarenteed win)
...
echo Start battle with same player (kienboec)
start /b "kienboec battle" curl -i -X POST http://localhost:10001/battles --header "Authorization: Bearer kienboec-mtcgToken"
start /b "kienboec battle" curl -i -X POST http://localhost:10001/battles --header "Authorization: Bearer kienboec-mtcgToken"
ping localhost -n 10 >NUL 2>NUL
echo. 
echo "Should return HTTP 403 - Forbidden, player already in the lobby or in a battle"
echo.

echo Start battle between kienboec and altenhof
start /b "altenhof battle" curl -i -X POST http://localhost:10001/battles --header "Authorization: Bearer altenhof-mtcgToken"
ping localhost -n 10 >NUL 2>NUL
echo ========================================================================================
...
echo 25) Show all friends/requests of the user
...
echo.
echo "Should return HTTP 201 - Friend request sent successfully."
echo.
...
echo 27) Accept a friend request
...
echo.
echo "Should return HTTP 404 - User not found"
echo.
...
curl -i -X POST http://localhost:10001/friends/accept/altenhof --header "Authorization: Bearer kienboec-mtcgToken"
echo.
echo "Should return HTTP 404 - No pending Friend Request found"
echo.
echo ========================================================================================
